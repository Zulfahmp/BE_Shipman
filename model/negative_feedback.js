...
router.post('/add-negative-feedback',validation,async(req,res)=>{
    ...
    try {
        let nf = await DBP.query('Insert into production.negative_feedback(nf_id,ship_id,port_id,port_next_id,created_by,status_nf) values($1,$2,$3,$4,$5,$6)',[b.nf_id,b.ship_id,b.port_id,b.port_next_id,b.created_by.toUpperCase(),-1])
        let detail = b.detail.map(a => [a.id_nf, a.ref_number, a.remarks, a.evidence_name,-1]);
        const query = formatQ(
        'INSERT INTO production.detail_negative_feedback(id_nf, ref_number, remarks, evidence,status_feedback) VALUES %L',detail);
...
router.get('/list-negative-feedback',validation,async(req,res)=>{
    try {
        ...
        let restotal = await DBP.query(`select count(1) total from production.negative_feedback n ${join} where ${searching}`,[`%${search}%`])

        let result = await DBP.query(`select nf_id,mship.ship_name,mport.port_name,port_next.port_name port_next_name,approver,to_char(n.created_at,'YYYY-MM-DD') date_of_report,status_nf from production.negative_feedback n ${join} where ${searching} order by n.created_at desc limit $2 offset $3`,[`%${search}%`,req.query.limit,req.query.current_page])
        res.status(200).json({lists:result.rows,total_rows:restotal.rows[0]["total"]})
    } catch (error) {
...
router.post('/nf-approval',validation,async(req,res)=>{
    ...
    try {
        let {rowCount} = await DBP.query("UPDATE production.negative_feedback set status_nf=$1,approver=$2 where nf_id=$3",[b.status_nf,tokenDecode.full_name,b.nf_id])
        b.detail.forEach(async(a)=>{
            await DBP.query("UPDATE production.detail_negative_feedback set status_feedback=$1 where id=$2",[a.status_feedback,a.id])
        })
...
router.post('/negative-feedback-detail',validation,async(req,res)=>{
    try {
        let join = `
            ...
            left join production.master_port port_next on n.port_next_id=port_next.port_id
        `
        let nfmaster = await DBP.query(`select mship.ship_name,mport.port_name,port_next.port_name port_next_name,created_by,approver,n.created_at,status_nf from production.negative_feedback n ${join} where nf_id=$1`,[req.body.nf_id])
...
router.get('/nf-view/:nf_id',validation,async(req,res)=>{
    try {        
        let join = `
            ...
            left join production.master_port port_next on n.port_next_id=port_next.port_id
        `
        let result = await DBP.query(`select *,mship.ship_name,mport.port_name,port_next.port_name port_next_name,to_char(n.created_at,'YYYY-MM-DD') n.created_at,status_nf from production.negative_feedback where nf_id=$1 limit 1`,[req.params.nf_id])
        let detail = await DBP.query(`select * from production.detail_negative_feedback where id_nf=$1 order by ref_number`,[req.params.nf_id])
