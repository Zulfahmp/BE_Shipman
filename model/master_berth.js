...
router.get("/total-master-berth", validation, async (req, res) => {
  try {
    let { rows } = await DBP.query(
      "select count(1) total from production.master_berth where deleted_at is NULL limit 1",
    );
...
router.get("/list-master-berth", validation, async (req, res) => {
  try {
    ...
    let { rows } = await DBP.query(
      `select berth_id,berth_name,berth_code from production.master_berth where deleted_at IS NULL ${where} order by berth_name`,
    );
...
router.post("/add-master-berth", validation, async (req, res) => {
  try {
    let { rowCount } = await DBP.query(
      "Insert into production.master_berth(berth_name,berth_code) values($1,$2)",
      [req.body.berth_name.toUpperCase(), req.body.berth_code],
    );
...
router.post("/edit-master-berth", validation, async (req, res) => {
  try {
    let { rowCount } = await DBP.query(
      "UPDATE production.master_berth set berth_name=$1,berth_code=$2 where berth_id=$3",
      [
        req.body.berth_name.toUpperCase(),
        req.body.berth_code,
        req.body.berth_id,
      ],
    );
...
router.post("/delete-master-berth", validation, async (req, res) => {
  try {
    ...
    let { rowCount } = await DBP.query(
      "UPDATE production.master_berth set deleted_at=$1 where berth_id=$2",
      [timestamp, req.body.berth_id],
    );
    res.status(200).json({
      affected: rowCount,
      timestamp: timestamp,
      id: req.body.berth_id,
    });
  } catch (error) {
    res.status(200).json({ affected: -1, message: error });
  }
