...
router.get('/list-master-berth',validation,async(req,res)=>{
    try {
        let {rows} = await DBP.query('select berth_id,berth_name,berth_code from production.master_berth where deleted_at IS NULL')
...
router.post('/add-master-berth',validation,async(req,res)=>{
    try {
        let {rowCount} = await DBP.query('Insert into production.master_berth(berth_name,berth_code) values($1,$2)',[req.body.berth_name,req.body.berth_code])
...
router.post('/edit-master-berth',validation,async(req,res)=>{
    try {
        let {rowCount} = await DBP.query('UPDATE production.master_berth set berth_name=$1,berth_code=$2 where berth_id=$3',[req.body.berth_name,req.body.berth_code,req.body.berth_id])
