...
router.get('/list-users',validation,async(req,res)=>{
    try {
        ...
        let {rows} = await DBP.query(`select user_id,full_name,email,role from production.users where deleted_at IS NULL AND (token_verifier IS NULL OR token_verifier!=$1) ${where} order by full_name`,[req.headers.authorization.split(' ')[1]])
...
router.post('/add-user',validation,async(req,res)=>{
    try {
        ...
        let {rowCount} = await DBP.query('INSERT into production.users(full_name,email,password,role,position) values($1,$2,$3,$4,$5)',[req.body.full_name.toUpperCase(),req.body.email.toLowerCase(),pass.hash,req.body.role,{1:'Admin',2:'Verifier',3:'Checker'}[req.body.role]])
        if(rowCount){
            let html=`
            ...
            res.status(200).json({affected:rowCount,pass:req.body.password||"#Pertamina321"})
        }else{
...
router.post('/edit-user',validation,async(req,res)=>{
    try {
        let {rowCount} = await DBP.query('UPDATE production.users set full_name=$1,email=$2,role=$3 where user_id=$4',[req.body.full_name.toUpperCase(),req.body.email.toLowerCase(),req.body.role,req.body.user_id])
...
router.post('/change-password',validation,async(req,res)=>{
    try {
        ...
        let {rows} = await DBP.query('select password from production.users where email=$1',[tokendata.email])
        // res.send(rows)
        // return
        ...
        if(passcheck && req.body.new_password==req.body.confirm_password){
            ...
            let {rowCount} = await DBP.query('UPDATE production.users set password=$1 where email=$2',[new_password,tokendata.email])
            res.status(200).json({affected:rowCount,message:'Update Successful!'})
        }else{
            res.status(200).json({affected:-2,message:'Old Password Not Matched!'})
        }
    } catch (error) {
        res.status(200).json({affected:-1,message:'Update Failed!',error:error})
    }
