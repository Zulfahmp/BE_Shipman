...
router.get('/total-master-port',validation,async(req,res)=>{
    try {
        let {rows} = await DBP.query('select count(1) total from production.master_port where deleted_at is NULL limit 1')
...
router.post('/delete-master-port',validation,async(req,res)=>{
    try {
        ...
        let {rowCount} = await DBP.query('UPDATE production.master_port set deleted_at=$1 where port_id=$2',[timestamp,req.body.port_id])
        res.status(200).json({affected:rowCount,timestamp:timestamp,id:req.body.port_id})
    } catch (error) {
        res.status(200).json({affected:-1,message:error})
    }
})
