...
router.post('/authorization-checking', validation,async (req, res) =>{
    ...
    try {
        ...
        if(is_valid){
            ...
            try {
                await DBP.query(`UPDATE production.users set token_verifier=$1 where email=$2`,[token,user.email])
            } catch (error) {   
                console.log(error)
            }
        }else{
...
router.post('/authorization-validation',validation,async(req,res)=>{
    let {rows} = DBP.query('select token_verifier from production.users where token=$1',[req.token])
    rows.length ? res.json({authenticated:true}) : res.json({authenticated:false})
})
