...
router.get("/total-master-cargo", validation, async (req, res) => {
  try {
    let { rows } = await DBP.query(
      "select count(1) total from production.master_cargo where deleted_at is NULL limit 1",
    );
...
router.get("/list-master-cargo", validation, async (req, res) => {
  try {
    ...
    if (req.query.search && /^[^=!;]*$/.test(req.query.search)) {
      const columns = ["cargo_name", "cargo_code"];
    ...
    let { rows } = await DBP.query(
      `select cargo_id,cargo_name,cargo_code from production.master_cargo where deleted_at IS NULL ${where} order by cargo_name`,
    );
...
router.post("/add-master-cargo", validation, async (req, res) => {
  try {
    let { rowCount } = await DBP.query(
      "Insert into production.master_cargo(cargo_name,cargo_code) values($1,$2)",
      [req.body.cargo_name.toUpperCase(), req.body.cargo_code],
    );
...
router.post("/edit-master-cargo", validation, async (req, res) => {
  try {
    let { rowCount } = await DBP.query(
      "UPDATE production.master_cargo set cargo_name=$1,cargo_code=$2 where cargo_id=$3",
      [
        req.body.cargo_name.toUpperCase(),
        req.body.cargo_code,
        req.body.cargo_id,
      ],
    );
...
router.post("/delete-master-cargo", validation, async (req, res) => {
  try {
    ...
    let { rowCount } = await DBP.query(
      "UPDATE production.master_cargo set deleted_at=$1 where cargo_id=$2",
      [timestamp, req.body.cargo_id],
    );
    res.status(200).json({ affected: rowCount });
  } catch (error) {
    res.status(200).json({ affected: -1, message: error });
